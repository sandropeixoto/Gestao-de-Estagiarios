...
class Contract extends BaseModel {
    ...
    public function allWithDetails() {
        $sql = "SELECT c.*, s.nome as estagiario, i.razao_social as instituicao, 
                       ch.descricao as carga_horaria, n.descricao as nivel,
                       m.descricao as motivo_desligamento,
                       sup.nome as supervisor_name
                FROM contracts c
                LEFT JOIN students s ON c.student_id = s.id
                LEFT JOIN institutions i ON c.institution_id = i.id
                LEFT JOIN cargas_horarias ch ON c.carga_horaria_id = ch.id
                LEFT JOIN niveis_escolaridade n ON c.nivel_escolaridade_id = n.id
    ...
    public function create($data) {
        ...
        $sql = "INSERT INTO contracts (
                    ...
                    nivel_escolaridade_id, carga_horaria_id, data_inicio, data_fim, 
                    valor_bolsa, valor_transporte, apolice_seguro, status
                ) VALUES (
                    :student_id, :institution_id, :supervisor_id, :position_id, 
                    :nivel_escolaridade_id, :carga_horaria_id, :data_inicio, :data_fim, 
                    :valor_bolsa, :valor_transporte, :apolice_seguro, :status
